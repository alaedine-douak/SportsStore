...
public class HomeControllerTests
{
    ...
    [Fact]
    public void Generate_Catogory_Specific_Product_Count()
    {
        // arrange
        Mock<IStoreService> mock = new();
        mock.Setup(x => x.Produts).Returns((new Product[] 
        ...
        // action
        int? res1 = GetModel((ViewResult)target.Index("Cat1"))?.PageInfo.TotalItems; 
        int? res2 = GetModel((ViewResult)target.Index("Cat2"))?.PageInfo.TotalItems; 
        int? res3 = GetModel((ViewResult)target.Index("Cat3"))?.PageInfo.TotalItems; 
        int? resAll = GetModel((ViewResult)target.Index(null))?.PageInfo.TotalItems; 
